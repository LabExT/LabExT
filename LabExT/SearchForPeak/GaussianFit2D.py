...
from LabExT.Measurements.MeasAPI.Measparam import MeasParamInt, MeasParamFloat, MeasParamList
...
class GaussianFit2D(PeakSearcher):
    """

    """
    name = "GaussianFit2D"

    DIMENSION_NAMES_TWO_STAGES = ['Input X', 'Input Y', 'Output X', 'Output Y']
    ...
    def fit_gaussian(self, x_data, y_data):
        ...
        pinit = GaussianFit2D._gaussian_param_initial_guess(x_data, y_data)
        ...
        popt, cov = curve_fit(GaussianFit2D._gaussian,
                              x_data,
                              y_data,
                              p0=pinit,
    ...
    def _raise_if_instr_or_stage_missing(self) -> None:
        ...
        if self.input_calibration is None and self.output_calibration is None:
            raise RuntimeError(
                "The Search for Peak requires at least one left or right stage configured.")

    def algortihm(self, data, instruments, stages,
                  parameters) -> Type[StageCoordinate]:
        ...
        """
        self.instr_powermeter = instruments['Power Meter']
        self.instr_laser = instruments['Laser']

        self.input_calibration = self.mover.input_calibration
        self.output_calibration = self.mover.output_calibration

        self._raise_if_instr_or_stage_missing()

        if self.input_calibration and self.output_calibration:
        ...
        for param_name, cfg_param in parameters.items():
            data['parameter'][param_name] = str(
                cfg_param.value) + str(cfg_param.unit)

        # send user specified parameters to instruments
        self.instr_laser.wavelength = parameters['Laser wavelength'].value
        self.instr_laser.power = parameters['Laser power'].value
        self.instr_powermeter.unit = 'dBm'
        self.instr_powermeter.wavelength = parameters['Laser wavelength'].value
        self.instr_powermeter.range = parameters['Power Meter range'].value
        ...
        with self.instr_laser:
            with self.mover.set_stages_coordinate_system(CoordinateSystem.STAGE):
                ...
                sfp_type = parameters.get('SfP type').value
                radius_us = parameters.get('Search radius').value

                # parameters specifically for stepped sfp
                stepsize_us = parameters['(stepped SfP only) Search step size'].value
                pause_time_ms = parameters['(stepped SfP only) Search fiber stabilization time'].value

                # parameters specifically for swept SfP
                t_sweep = parameters.get('(swept SfP only) Search time').value
                no_points = int(
                    parameters.get('(swept SfP only) Number of points').value)
                ...
                # SFP
                _input_start_coordinates = []
                _output_start_coordinates = []
                if self.input_calibration:
                    _input_start_coordinates = self.input_calibration.get_position().to_list()[
                        :2]
                if self.output_calibration:
                    _output_start_coordinates = self.output_calibration.get_position().to_list()[
                        :2]
                start_coordinates = _input_start_coordinates + _output_start_coordinates
                ...
                data['start location'] = start_coordinates.copy()
                data['start through power'] = self.instr_powermeter.power
                ...
                for dimidx, p_start in enumerate(start_coordinates):
                    ...
                    # 1st decision: did the power meter always return useful
                    # data?
                    if ~np.all(np.isfinite(IL_meas)):
                        ...
                        try:
                            ...
                            popt = GaussianFit2D._gaussian_param_initial_guess(
                                d_range, IL_meas)
                        ...
                        # plot the gaussian, if gaussian was successfully
                        # fitted
                        if perr_std_dev is not None:
                            ...
                            d_range_highres = np.linspace(
                                d_range.min(), d_range.max(), num=len(
                                    meas_plot.x) * 5)
                            IL_fit_fctn = GaussianFit2D._gaussian(
                    ...
                    data['fitting information'][dimension_name] = {
        ...
        data['optimized location'] = current_coordinates.copy()
        data['optimized through power'] = estimated_through_power

    def _move_stages_absolute(self, coordinates: list):
        with self.mover.set_stages_coordinate_system(CoordinateSystem.STAGE):
            if self.input_calibration and self.output_calibration:
                leftz = self.input_calibration.get_position().z
                rightz = self.output_calibration.get_position().z
            ...
            elif self.input_calibration:
                leftz = self.input_calibration.get_position().z
                assert len(coordinates) == 2
                self.input_calibration.move_absolute(
                    StageCoordinate.from_list(coordinates + [leftz]))
            elif self.output_calibration:
                rightz = self.output_calibration.get_position().z
                assert len(coordinates) == 2
                self.output_calibration.move_absolute(
