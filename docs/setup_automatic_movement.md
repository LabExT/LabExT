# Mover Setup: Automatically Move Connected Stages
...
Calibrations can assume different states. Depending on the state, different functions are available at the calibration level.
...
2.  In the wizard step "Driver Settings" all stage implementations are displayed. These are loaded from LabExT Core and Addon Paths.
    Make sure that the stage you want to use has an implementation and its drivers are loaded. If not, check the Troubleshooting. Click "Next Step".
...
3.  In the "Stage Connection" wizard step, found stages are selected for later use. For this purpose, an orientation, device port, and stage type must be specified for each stage. 
    -   The **Orientation** indicates the approximate position of the stage in the chip plane.
    ...
    -   The **Stage Type** is used to support different stage types and their shapes. Your stage type is not displayed? You can follow the guide here to extend LabExT with your stage type.
...
4.  Define all properties for all desired stages. Then, if prompted, set the settings of the stage polygon. This tells the PathFinding algorithm how big the stage is.
...
The overall status of the mover is also displayed. If everything goes without problems, all stages and the Mover are in `COORDINATE SYSTEM FIXED` status.
...
!!! note
    Familiarize yourself with the meaning of possible edge cases of your stage. For example, a speed of $0 \frac{um}{s}$ and acceleration of $0 \frac{um}{s^2}$ for SmarAct Stage means disabling the control feature.
...
The saved assignment is automatically applied if a new stage is added in the "Configure Stages" wizard with the same identifier.
...
!!! note
    ...
    You can follow this guide to implement a new algorithm.

## Single Stage Algorithms
The [Single Stage Planning](/LabExT/Movement/PathPlanning.py#SingleStagePlanning) is used when only one stage is registered.
...
The [Collision Avoidance Planning](/LabExT/Movement/PathPlanning.py#CollisionAvoidancePlanning) uses at least two stages
...
Use [this guide] to support new polygons in LabExT.


## Troubleshooting
### My stage class is not displayed in the *Configure Stages* Wizard.
LabExT loads all stage implementations in LabExT Core from the [`Stages`](/LabExT/Movement/Stages) subfolder.
...
Also, note that the class must inherit from `LabExT.Movement.Stage.Stage` and implement the interface correctly; more about this in [this guide].
...
Possibly there is no implementation for this stage type. Follow [this guide] to create an implementation.
